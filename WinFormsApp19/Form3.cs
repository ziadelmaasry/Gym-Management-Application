...
namespace WinFormsApp19
{
    public partial class AddMember : Form
    {
        ...
        SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=D:\Gym Application\database\GymDB.mdf;Integrated Security=True;Connect Timeout=30");
        ...
        {
            //"SHOWING TOTAL PRICE IN A TEXT BOX"
            try
            {
                Memberprice x = new Memberprice();
                int Month, Result;
                Month = Convert.ToInt32(Nmonthes.Text);
                if (Month>0)
                {                   
                    Result = x.Price(Month);
                    AmountTb.Text = Result.ToString();
                }
                else
                {
                    MessageBox.Show("Enter Correct Number of Months");
                }
            }
            catch (Exception Ex)
            {
